...
public sealed class WindowsConfigFolderPicker : IConfigFolderPicker
{
    ...
    {
        return MainThread.InvokeOnMainThreadAsync(() => PickFolderOnUiThread(title, okButtonLabel, cancellationToken));
    }

    private static string? PickFolderOnUiThread(string? title, string? okButtonLabel, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            dialog.SetTitle(string.IsNullOrWhiteSpace(title) ? "Chọn thư mục config" : title);
            dialog.SetOkButtonLabel(string.IsNullOrWhiteSpace(okButtonLabel) ? "Chọn thư mục" : okButtonLabel);
            dialog.SetFileNameLabel("Thư mục");
