...
public sealed class ConfigToolHub : Hub
{
    ...
    public async Task<GitFileExplorerDto> GetGitFileExplorerAsync(string folderPath, string? search)
    {
        return await RunGitTabOperationAsync("explorer", cancellationToken => _gitAdminService.GetFileExplorerAsync(folderPath, search, cancellationToken));
    }

    public async Task<GitRemoteExplorerDto> GetGitRemoteExplorerAsync(string folderPath, string? remoteName, string? branchName, string? search)
    {
        return await RunGitTabOperationAsync("explorer", cancellationToken => _gitAdminService.GetRemoteExplorerAsync(folderPath, remoteName, branchName, search, cancellationToken));
    }
